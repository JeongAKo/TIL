...
class CategoryTabBarViewController: UIViewController {
  ...
  private lazy var borderView: UIView = {
    ...
    borderView.backgroundColor = #colorLiteral(red: 0.8666666667, green: 0.8666666667, blue: 0.8666666667, alpha: 1)  // MARK: - 컬러 변경
  ...
  private func categoryDidScroll() {
    self.categoryTabBarView.categoryDidScroll = { [weak self] scrollView in
      guard let self = self else { return print("weak reference error") }
  ...
  private func categoryViewDidEndScroll() {
    categoryView.categoryViewDidEndScroll = { [weak self] itemAt in
      guard let self = self else { return print("weak reference error") }
